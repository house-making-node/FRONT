...
function Step2Page() {
  ...
  const handleOptionChange = () => {
    ...
    setIsNextEnabled(!!selectedOption);
  };
  ...
  const handleNext = async () => {
    ...
    try {
      const response = await axios.patch(
        "http://3.36.240.5:3000/consulting/requirements/mood",
        {
  ...
  const handlePrevious = () => {
    navigate(`/consulting/${consultingId}/step1`);
  };
          ...
          <form id="step2Form">
            <div className="option">
              <MoodLabel id="moodLabel">
                <span id="userNameMood">{userName}</span>님이 좋아하는 분위기를
                선택해 주세요.
              </MoodLabel>
            </div>
            <OptionsContainer>
